...
char* getMeaning(struct Node* node, char* key) {

	if (node) {
		if (strcmp(node->key, key) > 0)	return getMeaning(node->left, key);
		else if (strcmp(node->key, key) < 0) return getMeaning(node->right, key);
		return node->value;
	}
	return "\0";
}